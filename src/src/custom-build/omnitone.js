...
(${performance.now().toFixed(2)}ms)`;
  ...
  function FOARenderer(context, config) {
    ...
    if (config && config.hrirBufferList) {
      if (config.hrirPathList) {
        Utils.throw(
            'FOARenderer: Multiple HRIR options. Either use path list ' +
      ...
      if (Array.isArray(config.hrirBufferList) &&
        ...
        Utils.throw(
            'FOARenderer: Invalid HRIR Buffer. It must be an array with ' +
            '2 HRIR buffers. (got ' + config.hrirBufferList + ')');
      }
    }
...
export default class HOARotator {
    ...
    constructor(context, order) {
        ...
        this.in = this.context.createChannelSplitter(this.nCh);
        this.out = this.context.createChannelMerger(this.nCh);
  ...
  */
  ...
  function HOARenderer(context, config) {
    ...
    if (config && config.hrirBufferList) {
      ...
      if (Array.isArray(config.hrirBufferList) &&
        ...
        Utils.throw(
            'HOARenderer: Invalid HRIR Buffer. It must be an array with ' +
            this._config.numberOfStereoChannels + ' HRIR buffers.' +
            ' (got ' + config.hrirBufferList + ')');
      }
    }
    if (config && config.renderingMode) {
  ...
  HOARenderer.prototype._initializeCallback = function(resolve, reject) {
    ...
    if (this._config.pathList) {
      ...
      bufferList = new BufferList(this._context, OmnitoneTOAHrirBase64);
    }
    bufferList.load().then(
        function(hrirBufferList) {
